...
public class TORequestsTest {
	...
	private static final String RUNNER_RESULT_INFO_LIST_REQUEST = "SELECT RS.POSITION AS ABSPOSITION, "
			...
			+ " ON RC.PARK=PK.ID LEFT JOIN CLUBS CL ON RS.CLUB = CL.ID WHERE RS.RUNNER =";
	...
	private final static String RUNNERRACERESULTREQUESTFORMEN = "SELECT RS.POSITION AS POSITION, RN.FIRSTNAME AS FIRSTNAME, "
			+ "RN.LASTNAME AS LASTNAME, CL.NAME AS CLUBNAME, CL.SMALLLOGO AS CLUBLOGO, CL.ID AS CLUBID, "
			+ "RN.DATEOFBIRTH AS DATEOFBIRTH, RS.TIME AS TIME, RN.ID AS RUNNERID, PC.LOCATION AS AVATAR "
			+ "FROM RESULTS RS LEFT JOIN RUNNERS RN ON RS.RUNNER=RN.ID LEFT JOIN CLUBS CL ON RS.CLUB=CL.ID"
			+ " LEFT JOIN PICTURES PC ON RN.LOGO=PC.ID WHERE RS.RACE = 223 AND RS.GENDER = 'M'";
	private final static String RUNNERRACERESULTREQUESTFORWOMEN = "SELECT RS.POSITION AS POSITION, "
			+ "RN.FIRSTNAME AS FIRSTNAME, RN.LASTNAME AS LASTNAME, CL.NAME AS CLUBNAME, CL.SMALLLOGO "
			+ "AS CLUBLOGO, CL.ID AS CLUBID, RN.DATEOFBIRTH AS DATEOFBIRTH, RS.TIME AS TIME, RN.ID AS RUNNERID, "
			+ "PC.LOCATION AS AVATAR FROM RESULTS RS LEFT JOIN RUNNERS RN ON RS.RUNNER=RN.ID LEFT JOIN CLUBS CL ON "
			+ "RS.CLUB=CL.ID LEFT JOIN PICTURES PC ON RN.LOGO=PC.ID WHERE RS.RACE = 223 AND RS.GENDER = 'F'";
