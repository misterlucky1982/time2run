...
public class TORequestsTest {
	...
	private static final String CLUB_RACE_RESULT_TO_REQUEST = "SELECT RS.POSITION "
			+ "AS ABSPOSITION,(SELECT COUNT(*)+1 FROM RESULTS WHERE RACE=RS.RACE AND "
			+ "POSITION<RS.POSITION AND GENDER = RS.GENDER) AS POSITIONINGENDERGROUP,RC.DATE AS RACE_DATE, "
			+ "RC.NAME AS RACENAME, PK.NAME AS PARK_NAME, RC.ID AS RACE_ID,RN.FIRSTNAME AS FIRSTNAME, "
			+ "RN.LASTNAME AS LASTNAME, RN.ID AS RUNNERID, RS.GENDER AS GENDER, RS.TIME AS TIME "
			+ "FROM RESULTS RS INNER JOIN RACES RC ON RS.RACE = RC.ID INNER JOIN PARKS PK ON "
			+ "RC.PARK=PK.ID INNER JOIN RUNNERS RN ON RS.RUNNER=RN.ID WHERE RS.CLUB=";
	...
	private static final String RACE_TO_REQUEST = "SELECT RC.ID AS RACE_ID, RC.DATE AS RACE_DATE, RC.NAME AS RACENAME, "
	...
	private final String RACE_TO_REQUEST_FOR_LAST_RACE = "SELECT RC.ID AS RACE_ID, RC.DATE AS RACE_DATE, "
			+ "RC.NAME AS RACENAME, PK.NAME AS PARK_NAME, (SELECT COUNT(RT.ID) FROM RESULTS RT WHERE"
			+ " RT.RACE=RC.ID AND RT.GENDER='M') AS M_PARTICIPANTS, (SELECT COUNT(RT.ID) FROM RESULTS RT "
			+ "WHERE RT.RACE=RC.ID AND RT.GENDER='F') AS W_PARTICIPANTS FROM RACES RC INNER JOIN PARKS PK ON "
			+ "RC.PARK=PK.ID ORDER BY RC.DATE DESC, RC.ID DESC LIMIT 1;";

	private final static String FULL_RACE_TO_LIST_REQUEST = "SELECT RC.ID AS RACE_ID, RC.DATE AS RACE_DATE,"
			+ " RC.NAME AS RACENAME, PK.NAME AS PARK_NAME FROM RACES"
			+ " RC INNER JOIN PARKS PK ON RC.PARK=PK.ID ORDER BY RC.DATE DESC, RC.ID DESC;";
	
	private final static String FULL_RACE_TO_LIST_FOR_PARK_REQUEST = "SELECT RC.ID AS RACE_ID, "
	...
	private final static String FULL_RACE_TO_REQUEST_01 = "SELECT RC.ID AS RACE_ID,"
	...
	private final static String FULL_RACE_TO_REQUEST_02 = "SELECT RC.ID AS RACE_ID, RC.DATE AS RACE_DATE, "
	...
	private final static String FULL_RACE_TO_REQUEST_03 = "SELECT RC.ID AS RACE_ID, RC.DATE AS RACE_DATE, "
	...
	private static final String RUNNER_RESULT_INFO_LIST_REQUEST_0 = "SELECT RS.POSITION AS ABSPOSITION, "
			+ "(SELECT COUNT(*)+1 FROM RESULTS WHERE RACE=RS.RACE AND POSITION<RS.POSITION AND GENDER = "
			+ "RS.GENDER) AS POSITIONINGENDERGROUP,RS.CLUB AS CLUBID,CL.NAME AS CLUBNAME, CL.SMALLLOGO "
			+ "AS CLUBLOGO, RS.RACE AS RACE_ID, RC.DATE AS RACE_DATE, RC.NAME AS RACENAME, "
			+ "PK.NAME AS PARK_NAME, RS.TIME AS TIME FROM RESULTS RS INNER JOIN RACES RC ON RS.RACE=RC.ID "
			+ "INNER JOIN PARKS PK ON RC.PARK=PK.ID LEFT JOIN CLUBS CL ON RS.CLUB = CL.ID WHERE RS.RUNNER =1 "
			+ "ORDER BY RC.DATE DESC, RC.ID DESC;";
	...
	public void raceTOReqestForLastRaceTest(){
		assertEquals(RACE_TO_REQUEST_FOR_LAST_RACE,TORequests.extendedRaceTORequestForLastRace());
	}
