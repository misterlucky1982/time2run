...
public class TORequests {
	...
	private static final String RUNNER_RACE_RESULT_REQUEST = "SELECT RS.POSITION AS "+POSITION+", RN.FIRSTNAME AS "+FIRSTNAME
			...
			+", RN.DATEOFBIRTH AS "+DATEOFBIRTH+", RS.TIME AS "+TIME+", RN.ID AS "+RUNNERID+", RN.LOGO AS "+AVATAR+
			" FROM RESULTS RS LEFT JOIN RUNNERS RN ON RS.RUNNER=RN.ID LEFT JOIN CLUBS CL ON RS.CLUB=CL.ID WHERE RS.RACE = &&RACE&& AND RS.GENDER = '&&GENDER&&'";
